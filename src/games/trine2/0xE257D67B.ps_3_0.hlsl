...
float4 main(PS_IN i) : COLOR {
  ...
  float2 glow_step = SCREEN_SIZE.zw;

  float3 glow_accum =
      tex2D(GLOW_HDR_SOURCE_TEXTURE, glow_texcoord + glow_step).rgb +
      tex2D(GLOW_HDR_SOURCE_TEXTURE, glow_texcoord + glow_step * float2(-1.f, 1.f)).rgb +
      tex2D(GLOW_HDR_SOURCE_TEXTURE, glow_texcoord + glow_step * float2(1.f, -1.f)).rgb +
      tex2D(GLOW_HDR_SOURCE_TEXTURE, glow_texcoord - glow_step).rgb;

  float3 glow_center = tex2D(GLOW_HDR_SOURCE_TEXTURE, glow_texcoord).rgb;
  ...
  // This pass lerps an SDR-authored glow source toward white. In an upgraded
  // float path, the blend factor can exceed 1 and invert channel contribution,
  // which shows up as dark chromatic fringes around bright lanterns.
